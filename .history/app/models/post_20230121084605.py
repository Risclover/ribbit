from .db import db
import datetime
from .joins import post_upvotes, post_downvotes
...
class Post(db.Model):
    ...
    post_upvoters = db.relationship('User', back_populates="user_post_upvotes", secondary=post_upvotes, lazy="joined")
    post_downvoters = db.relationship("User", back_populates="user_post_downvotes", secondary=post_downvotes, lazy="joined")
    ...
    def to_dict(self):
        return {
            ...
            "postUpvoters": {item.to_dict()["id"]: item.to_dict() for item in self.post_upvoters},
            "postDownvoters": {item.to_dict()["id"]: item.to_dict() for item in self.post_downvoters},
