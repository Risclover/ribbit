...
from .joins import subscriptions, post_upvotes, post_downvotes
...
class User(db.Model, UserMixin):
    ...
    user_post_upvotes = db.relationship('Post', back_populates='post_upvoters', secondary=post_upvotes, lazy="joined")
    user_post_downvotes = db.relationship("Post", back_populates="post_downvoters", secondary=post_downvotes, lazy="joined")
    ...
    def to_dict(self):
        return {
            ...
            'email': self.email,
            'karma': self.karma,
            # "postVotes": {item.to_dict()["id"]: item.to_dict() for item in self.user_post_votes}
