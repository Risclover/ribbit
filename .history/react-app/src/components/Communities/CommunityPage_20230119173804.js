...
export default function CommunityPage() {
  ...
  let commPosts = posts.filter((post) => post.communityId == community[0]?.id);

  const dispatch = useDispatch();
  // const [loadedCommunity, setLoadedCommunity] = useState();
  const [isPage, setIsPage] = useState("community");
  const [community_id, setcommunity_id] = useState(community[0]?.id);
  ...
  useEffect(() => {
    dispatch(getSingleCommunity(communityName));
    dispatch(getPosts());
    // dispatch(getSubscriptions());
    dispatch(getSubscribers(community[0]?.id));
  }, [communityName, dispatch, community[0]?.id]);
                  ...
                  {!subscribed && (
                    <button
                      onClick={async (e) => {
                        ...
                        dispatch(addToSubscriptions(community[0].id));
                      }}
                    >
                      Join
                    </button>
                  ...
                  {user && subscribed && (
                    <button
                      ...
                      onClick={async (e) => {
                        e.preventDefault();
                        setSubscribed(false);
                        dispatch(deleteSubscription(community[0].id));
                      }}
                    >
                      Joined
                    </button>
          ...
          <div className="community-page-community-info">
            ...
            <div className="community-page-box-content">
              ...
              <div className="community-page-box-btn">
                {user && (
                  <NavLink to={`/c/${community[0].name}/submit`}>
                ...
                {user?.id === community[0].userId ? (
                  <NavLink to={`/c/${community[0].name}/edit`}>
