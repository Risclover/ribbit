  ...
  return (
    <div className="edit-community-page">
      ...
      <div className="delete-community">
        ...
        <p>
          Click the button below to delete this community. Please note that once
          you confirm deletion, you cannot undo this action.
        </p>
        <button className="delete-community-btn">
          Delete c/{community[0].name}
        </button>
        {showDeleteModal && <Modal onClose={() => setShowDeleteModal(faLse)} title={`Delete community ${community[0].name}?`}}
      </div>
    </div>
  );
}
