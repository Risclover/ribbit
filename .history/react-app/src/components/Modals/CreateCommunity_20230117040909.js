...
export default function CreateCommunity({
  ...
  const handleCreation = (e) => {
    ...
    if (name.length < 3 || name.length > 21 || !/^\w+$/.test(name)) errors.push("Community names must be between 3–21 characters, and can only contain letters, numbers, or underscores.")
    if(errors.length > 0 ){
        setErrors(errors);
    } else {
        ...
        if(data) {
            ...
            setShowCreateCommunityModal(false);l
        }
    }
  };
  return (
              ...
              <div className="modal-content-input">
                ...
                {errors.length > 0 &&
                  errors.map((error) => (
                    ...
                    <div>Community names must be between 3–21 characters, and can only contain letters, numbers, or underscores.</div>
                  ))}
              </div>
              <div className="modal-content-input">
                <h2>Description</h2>
