...
export default function UpdatePost({ setShowEditModal, showEditModal }) {
  ...
  const [title, setTitle] = useState(post[0]?.title.trim());
  const [content, setContent] = useState(post[0]?.content.trim());
  ...
  const handleSubmit = (e) => {
    ...
    if (titles.length > 0 || contents.length > 0) {
      ...
      const data = dispatch(putSinglePost({ title.trim(), content.trim() }, post[0].id));
      if (data.errors) {
    ...
    <div className="update-post-form-container">
      ...
      <form className="update-post-form" onSubmit={handleSubmit}>
        ...
        <div className="update-post-form-buttons">
          <button className="update-post-cancel">Cancel</button>
