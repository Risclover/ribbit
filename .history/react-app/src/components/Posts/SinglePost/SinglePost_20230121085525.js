...
export default function SinglePost({ id, isPage }) {
  ...
  useEffect(() => {
    for (let voter of Object.values(post.postUpvoters)) {
    ...
    for (let voter of Object.values(post.postDownvoters)) {
      if (voter.username === user.username) {
        setDownvoted(true);
        setUpvoted(false);
      }
        ...
        <div className="single-post-container">
          <div className="single-post-karmabar">
            <button
              onClick={async (e) => {
                e.preventDefault();
                upvoted && (await dispatch(removePostVote(post.id, "1")));
                downvoted && {(await dispatch(removePostVote(post.id, "0"))) await dispatch(addPostVote(post.id, "1"))}
                !upvoted &&
                  !downvoted &&
                  (await dispatch(addPostVote(post.id, "1")));
              }}
            >
              Like
            </button>
            ...
            <button
              onClick={async (e) => {
                e.preventDefault();
                downvoted && (await dispatch(removePostVote(post.id, "0")));
                upvoted && (await dispatch(removePostVote(post.id, "1")));
                !downvoted &&
                  !upvoted &&
                  (await dispatch(addPostVote(post.id, "0")));
              }}
            >
              Dislike
            </button>
