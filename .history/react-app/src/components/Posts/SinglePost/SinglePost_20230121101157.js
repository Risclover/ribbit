...
export default function SinglePost({ id, isPage }) {
  ...
  const [voteAllowed, setVoteAllowed] = useState(true);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < Object.values(post.postVoters).length; i++) {
      console.log("VOTERS:", Object.values(post.postVoters)[i]);
    ...
    console.log("vote allowed?", voteAllowed);
  }, [voteAllowed, post.postVoters]);
  console.log("vote allowed2?", voteAllowed);
