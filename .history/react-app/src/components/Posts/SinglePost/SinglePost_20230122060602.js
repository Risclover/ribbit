...
export default function SinglePost({ id, isPage }) {
  ...
  const postVoters = post?.postVoters;
  ...
  useEffect(() => {
    if (Object.values(postVoters).length === 0) {
      setVoteAllowed(true);
    } else {
      for (let voter of Object.values(postVoters)) {
  ...
  }, [voteAllowed, postVoters]);

  if (!post || !postVoters || !post.post) return null;
  return (
    <>
      {post && (
