...
export function useAuthFormInput(inputProps, blurred) {
  ...
  // Handles which type of validation icon is shown
  ...
  // Handler for Enter key; necessary so Enter key doesn't automatically submit form
  const enterKey = (e) => {
    if (e.key === "Enter") {
      return;
    }
